...
import { X, Download, Image as ImageIcon, Code } from 'lucide-react';
import Spinner from '../../Spinner';

interface ExportModalProps {
    isOpen: boolean;
    onClose: () => void;
    onExportCode: () => Promise<void>;
    onExportImage: () => Promise<void>;
}

const ExportModal: React.FC<ExportModalProps> = ({ isOpen, onClose, onExportCode, onExportImage }) => {
            ...
            >
                ...
                <div className="p-6 space-y-4">
                    <button
                        ...
                        disabled={isExportingCode || isExportingImage}
                    ...
                    >
                        {isExportingCode ? <Spinner /> : <Code size={24} className="flex-shrink-0" />}
                        <div className="text-left">
                            <p className="font-semibold">Download as Code</p>
                            <p className="text-xs text-zinc-500 dark:text-zinc-400">Export the current page as a single HTML file.</p>
                        </div>
                    </button>
                    <button
